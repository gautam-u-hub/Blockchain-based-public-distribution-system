...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  console.log(MetamaskAddress);
      const resa = await fetch(`http://localhost:5000/members`, {
        method: "POST",
        headers: {
          "content-type": "application/json",
        },
        body: JSON.stringify({ from: MetamaskAddress }),
      });
...
exports.loginUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const resa = await fetch(`http://localhost:5000/members`, {
    ...
    body: JSON.stringify({ from: user.MetamaskAddress }),
  });
  const data = await resa.json();
...
// check curr account nft
exports.checkCurrentAccountNft = catchAsyncErrors(async (req, res, next) => {
  const acc = req.body;
  console.log(acc.acc);
  const resa = await fetch(`http://localhost:5000/members`, {
    method: "POST",
    ...
    body: JSON.stringify({ from: acc.acc }),
  });
  const data = await resa.json();
  console.log(data.numNFTs.userNFTs + " hiii");
  // if (data.numNFTs.userNFTs <= 0) {
